...
RSpec.describe AuctionResultNotifier, type: :model do
  ...
  let(:auction) do
    create(:auction, seller: seller, current_highest_bid: bid,
                     end_time: 1.hour.ago)
  end
  let(:mailer) { double('AuctionResultMailer') }

  before do
    stub_const('AuctionResultNotifier::WEBHOOK_CONFIG',
               { auction_result_url: 'https://example.com',
                 auction_result_token: 'secret' })
  end
  ...
  describe '.notify' do
    it 'sends the webhook and emails' do
      ...
      allow(mailer).to receive_messages(winner_email: mailer,
                                        seller_email: mailer)
      allow(mailer).to receive(:deliver_later)

      described_class.notify(auction)

      expect(WebhookClient).to have_received(:send).with('https://example.com',
                                                         anything, hash_including('Authorization' => 'Bearer secret'))
      expect(AuctionResultMailer).to have_received(:with).with(
        auction: auction, winner: user, seller: seller,
      ).twice
