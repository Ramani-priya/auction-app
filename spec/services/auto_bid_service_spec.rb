  ...
  describe '#call' do
    ...
    context 'basic scenarios' do
      it 'does nothing if the triggering bid is system-generated' do
        create(:bid, auction: auction, user: user1, current_bid_price: 50,
                     max_bid_price: 100, system_generated: true)
        service = described_class.new(auction.id)
        expect(service.call).to be_nil
      end

      it 'does nothing if there are no eligible autobidders' do
        create(:bid, auction: auction, user: user1, current_bid_price: 50,
                     max_bid_price: 100, system_generated: false)
        service = described_class.new(auction.id)
        expect(service.call).to be_nil
      end
    end
    ...
    context 'single autobidder scenario' do
      ...
      before do
        create(:bid, auction: auction, user: user2, current_bid_price: 55,
                     max_bid_price: 120, autobid: true, system_generated: false)
        create(:bid, auction: auction, user: user1, current_bid_price: 60,
    ...
    context 'multiple autobidders scenarios' do
      it 'handles different max prices' do
        create(:bid, auction: auction, user: user1, current_bid_price: 50,
                     max_bid_price: 100, system_generated: false)
        create(:bid, auction: auction, user: user2, current_bid_price: 60,
                     max_bid_price: 160, autobid: true, system_generated: false)
        create(:bid, auction: auction, user: user3, current_bid_price: 70,
                     max_bid_price: 170, autobid: true, system_generated: false)
        service = described_class.new(auction.id)
        bid = service.call
      ...
      it 'resolves tie by creation time when max prices are equal' do
        create(:bid, auction: auction, user: user1, current_bid_price: 50,
                     max_bid_price: 100, system_generated: false)
        create(:bid, auction: auction, user: user2, current_bid_price: 60,
                     max_bid_price: 100, autobid: true, system_generated: false, created_at: 2.hours.ago)
        create(:bid, auction: auction, user: user3, current_bid_price: 70,
                     max_bid_price: 100, autobid: true, system_generated: false, created_at: 1.hour.ago)
    ...
    it "takes second highest's max price plus increment if available" do
      create(:bid, auction: auction, user: user1, current_bid_price: 50,
                   max_bid_price: 100, system_generated: false)
      create(:bid, auction: auction, user: user2, current_bid_price: 60,
                   max_bid_price: 130, autobid: true, system_generated: false)
      create(:bid, auction: auction, user: user3, current_bid_price: 70,
                   max_bid_price: 120, autobid: true, system_generated: false)
    ...
    it 'falls back to triggering bid price when second highest is not available and no other auto bids with max bid price > current bid price' do
      create(:bid, auction: auction, user: user1, current_bid_price: 50,
                   max_bid_price: 60, autobid: true, system_generated: false)
      create(:bid, auction: auction, user: user2, current_bid_price: 70,
                   max_bid_price: 120, autobid: true, system_generated: false)
    ...
    context 'edge case: same max bid, min increment exceeds cap' do
      it 'chooses the earliest created autobid and caps price at max_bid_price' do
        create(:bid, auction: auction, user: user1, current_bid_price: 50,
                     max_bid_price: 70, system_generated: false)
