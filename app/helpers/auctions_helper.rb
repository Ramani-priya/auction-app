...
module AuctionsHelper
  ...
  def auction_price(auction, show_price)
    ...
    content_tag(:p, "Current Price: #{auction_current_price(auction)}",
                class: 'card-price')
  end
  ...
  def auction_start_time(auction)
    content_tag(:p, formatted_time(auction.start_time))
  end

  def auction_end_time(auction)
    content_tag(:p, formatted_time(auction.end_time))
  end

  def allow_to_place_bid(auction, user)
    user_signed_in? && user != auction.seller && auction.in_progress?
  end
end
